import { g as sequence } from "./chunks/render_IYlne62i.mjs";
import * as fs from "node:fs";
import * as path from "node:path";
import { clerkJSScriptUrl, clerkUIScriptUrl } from "@clerk/shared/loadClerkJsScript";
import { createClerkClient } from "@clerk/backend";
import { AuthStatus, TokenType, constants, createClerkRequest, createRedirect, getAuthObjectForAcceptedToken, signedOutAuthObject } from "@clerk/backend/internal";
...
import { handleValueOrFn, isAutomatedEnvironment, isDevelopmentEnvironment } from "@clerk/shared/utils";
...
//#region node_modules/@clerk/astro/dist/async-local-storage.server.js
...
//#endregion
//#region node_modules/@clerk/astro/dist/server/index.js
...
function getContextEnvVar(envVarName, contextOrLocals) {
	...
	return Object.assign({
		...
		"SSR": true
	}, { _: "/root/.nvm/versions/node/v24.18.1/bin/npm" })[envVarName] || void 0;
}
...
var createClerkClientWithOptions = (context, options) => createClerkClient({
	...
	sdkMetadata: {
		...
		environment: Object.assign({
			...
			"SITE": "https://example.com",
			"SSR": true
		}, { _: "/root/.nvm/versions/node/v24.18.1/bin/npm" }).MODE
	},
	telemetry: {
...
var CONTROL_FLOW_ERROR = { REDIRECT_TO_SIGN_IN: "CLERK_PROTECT_REDIRECT_TO_SIGN_IN" };
var clerkMiddleware = (...args) => {
	const [handler, options] = parseHandlerAndOptions(args);
	const astroMiddleware = async (context, next) => {
		if (isPrerenderedPage(context)) return next();
		await initCloudflareEnv();
		const clerkRequest = createClerkRequest(patchRequest(context.request));
		let keylessClaimUrl;
...
var handleControlFlowErrors = (e, clerkRequest, requestState, context) => {
	...
	switch (e.message) {
		case CONTROL_FLOW_ERROR.REDIRECT_TO_SIGN_IN: return createRedirect({
			redirectAdapter,
...
var onRequest$1 = clerkMiddleware((auth, context, next) => {
	if (context.url.pathname.startsWith("/dashboard") && !auth().userId) return auth().redirectToSignIn();
	return next();
});
//#endregion
