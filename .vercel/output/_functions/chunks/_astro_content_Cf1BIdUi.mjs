import { U as RenderUndefinedEntryError, Z as UnknownContentCollectionError, tt as AstroError } from "./errors-data_CGrY2m_4.mjs";
import { c as isRemotePath, d as removeBase, u as prependForwardSlash } from "./path_DSAdOJJi.mjs";
import { M as unescapeHTML, O as createHeadAndContent, a as renderUniqueStylesheet, c as renderComponent, g as renderTemplate, i as renderScriptElement, r as spreadAttributes, y as generateCspDigest } from "./server_BygG88gc.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_Do6VFdv3.mjs");
...
function createGetCollection({ liveCollections }) {
	return async function getCollection(collection, filter) {
		...
		if (await store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_BlUbeoD8.mjs");
			const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
	...
	const { getImage } = await import("./_virtual_astro_get-image_D17rw1Sv.mjs");
...
async function renderEntry(entry) {
	...
	if (entry.deferredRender) try {
		const { default: contentModules } = await import("./content-modules_Cxh_K9T8.mjs");
